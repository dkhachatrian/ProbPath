...
A momentum function is also considered as a factor, assuming the agent has some inertia that overcomes diffusive tendencies. This is not an unreasonable assumption for agents at the size of cells, whose diffusivity in water could be estimated as D_cell_in_water \approx 10^(-11) cm2/s \approx 0.00009 mm2/day (assuming D ~ lambda**2, lambda ~ R, V ~ R**3, rho_protein \approx rho_cell, and knowing that D_protein_in_water \approx 10^(-9) cm2/s). Taking into account the much larger mass, it is unlikely that momentum effects can be ignored.
